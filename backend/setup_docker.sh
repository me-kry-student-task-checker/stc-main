...
DISCOVERY_SERVICE_PEER1="discovery-service-peer1"
DISCOVERY_SERVICE_PEER2="discovery-service-peer2"
...
docker image rm -f ${DISCOVERY_SERVICE_PEER1}
docker image rm -f ${DISCOVERY_SERVICE_PEER2}

echo -e "\n${YELLOW}Container deletion${NOCOLOR} \n"

docker container stop ${CONFIG_SERVICE}
docker container rm ${CONFIG_SERVICE}
docker container stop ${DISCOVERY_SERVICE_PEER1}
docker container rm ${DISCOVERY_SERVICE_PEER1}
docker container stop ${DISCOVERY_SERVICE_PEER2}
docker container rm ${DISCOVERY_SERVICE_PEER2}
...
docker build -f dockerfile-p1 -t ${DISCOVERY_SERVICE_PEER1} .
docker build -f dockerfile-p2 -t ${DISCOVERY_SERVICE_PEER2} .
cd ..
...
docker run --detach --name ${DISCOVERY_SERVICE_PEER1} --net ${NETWORK_NAME} -it ${DISCOVERY_SERVICE_PEER1}
docker run --detach --name ${DISCOVERY_SERVICE_PEER2} --net ${NETWORK_NAME} -it ${DISCOVERY_SERVICE_PEER2}